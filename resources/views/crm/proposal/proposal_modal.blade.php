  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content bg-light-gray">
      <div class="modal-header bg-gray">
        <h5 class="modal-title">#{{@$proposal->id}} |  {{__('Quotation')}} - {{$proposal->subject}} </h5>
...
{{-- ********* --}}
<span class="color1 width100" >{{__('Quotation')}} - #{{@$proposal->id}} <span class="floatRight">{{@$proposal->proposal_date}}</span></span>
...
{{__('Quotation description is not available!')}}
@else 
{{@$proposal->message}}    
@endif
<br>
...
@if (@$proposal->open_till_date != NULL)
  <p><span>{{__('Note')}}:</span>{{__('This quotation will expire on')}} {{@$proposal->open_till_date}}</p>
@endif
{{-- ********* --}}
      </div>
